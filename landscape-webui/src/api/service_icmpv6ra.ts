...
> {
  let data = await api.api.get(`services/icmpv6ra/status`);
...
export async function get_iface_icmpv6ra_config(
  ...
  let data = await api.api.get(`services/icmpv6ra/${iface_name}`);
...
export async function update_icmpv6ra_config(
  ...
  let data = await api.api.post(`services/icmpv6ra`, {
    ...icmpv6ra_config,
  });
  console.log(data.data);
  return data.data;
...
export async function stop_and_del_iface_icmpv6ra(name: string): Promise<void> {
  return api.api.delete(`services/icmpv6ra/${name}`);
}
