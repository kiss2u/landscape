...
> {
  let data = await api.api.get(`services/pppds/status`);
...
export async function get_all_iface_pppd_config(
  ...
  let data = await api.api.get(`services/pppds/${iface_name}`);
...
export async function stop_and_del_iface_pppd(name: string): Promise<void> {
  return api.api.delete(`services/pppds/${name}`);
}

export async function delete_and_stop_iface_pppd_by_attach_iface_name(
  attach_iface_name: string
): Promise<void> {
  return api.api.delete(`services/pppds/attach/${attach_iface_name}`);
}

export async function get_attach_iface_pppd_config(
  iface_name: string
): Promise<PPPDServiceConfig[]> {
  let data = await api.api.get(`services/pppds/attach/${iface_name}`);
  console.log(data.data);
  return data.data;
}
