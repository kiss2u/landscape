...
export async function get_geo_ip_configs(
  ...
  let data = await api.api.get(`config/geo_ips`, {
    params: {
      name,
    },
  });
...
export async function get_geo_ip_config(
  ...
  let data = await api.api.get(`config/geo_ips/${id}`);
  return data.data;
}
...
export async function get_geo_cache_key(
  ...
  let data = await api.api.get(`config/geo_ips/cache`, {
    params: { ...filter },
  });
  return data.data;
}
...
export async function refresh_geo_cache_key(): Promise<void> {
  let data = await api.api.post(`config/geo_ips/cache`);
}

export async function search_geo_ip_cache(
  query: QueryGeoKey
): Promise<GeoConfigKey[]> {
  let data = await api.api.get(`config/geo_ips/cache/search`, {
    params: {
      ...query,
    },
  });
...
export async function get_geo_ip_cache_detail(
  ...
  let data = await api.api.get(`config/geo_ips/cache/detail`, {
    params: {
      ...key,
    },
  });
