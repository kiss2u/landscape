use landscape_common::config::nat::StaticNatMapping;
...
pub fn add_static_nat_mapping(mappings: Vec<StaticNatMapping>) {
    if mappings.len() == 0 {
        return;
    }
    ...
    for static_mapping in mappings.into_iter() {
        ...
        let mut egress_mapping_key = static_nat_mapping_key {
            prefixlen: 192,
            port: static_mapping.target_port.to_be(),
        ...
        ingress_mapping_value.port = static_mapping.target_port.to_be();
        egress_mapping_value.port = static_mapping.port.to_be();
        ingress_mapping_value.is_static = 1;
        egress_mapping_value.is_static = 1;

        match static_mapping.target_ip {
