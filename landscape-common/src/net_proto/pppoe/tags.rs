        ...
        impl PPPoETag {
            pub fn from_bytes(data: &[u8]) -> Vec<PPPoETag> {
                ...
                loop {
                    ...
                    let tag_type = u16::from_be_bytes([data[start_index], data[start_index + 1]]);
                    let tag_len = u16::from_be_bytes([data[start_index + 2], data[start_index + 3]]);
                    let next_index = 4 + start_index + tag_len as usize;
                    if next_index > data.len() {
                        break;
                    }
                    let tag_data = if tag_len == 0 {
                        vec![]
                    } else {
                        data[start_index + 4..next_index].to_vec()
                    };
                    match tag_type {
                        $(
                            $num => {
                ...
                match self {
                    $(
                        PPPoETag::$name(value) => {
                            ...
                            [($num as u16).to_be_bytes().to_vec(), length.to_be_bytes().to_vec(), data]
                                .concat()
                        }
                    )*
